...
const moduleFactory = (require) => {
    ...
    /** 往屏里写一段文本(模拟终端:CR 回到行首覆盖,LF 下移)。 */
    function writeScreen(screen, text) {
      const clean = stripAnsi(text.replace(/\u001b/g, '\u001b'));
      for (let i = 0; i < clean.length; i += 1) {
        ...
        if (ch === '\r') {
          ...
          let line = screen.lines[screen.lines.length - 1];
          if (screen.col < line.length) {
            line = line.slice(0, screen.col) + ch + line.slice(screen.col + 1);
          } else {
            line = line + ' '.repeat(Math.max(0, screen.col - line.length)) + ch;
          }
          screen.lines[screen.lines.length - 1] = line;
          screen.col += 1;
        }
        if (screen.lines.length > MAX_LINES) {
          screen.lines.splice(0, screen.lines.length - MAX_LINES);
          screen.col = 0;
        }
      }
    }
...
`;
    ...
    return { apply, inject, name, createFeed, writeScreen, statusOf, commandLabel };
};
